...
#DATADOG_API_KEY = "xxxxx"  # remplace par ta vraie clé
...
def envoyer_vers_datadog(logs):
    """
    Envoie les logs vers Datadog, en recalant les timestamps sur les dernières heures
    (Datadog refuse les logs de plus de 18h dans le passé).
    """
    ...
    for i, log in enumerate(logs):
        # on étale les logs sur les 2 dernières heures, dans l'ordre
        timestamp_recent = maintenant - timedelta(minutes=(len(logs) - i) * 5)
        payload.append({
            "ddsource": "python-script",
            "service": log["service"],
            "status": log["level"].lower(),
            "message": log["message"],
            "ddtags": f"client_id:{log['client_id']},bank:{log.get('bank', 'unknown')}",
            "timestamp": timestamp_recent.isoformat(),
        })

    response = requests.post(url, headers=headers, json=payload)
    print(f"Statut envoi Datadog : {response.status_code}")
